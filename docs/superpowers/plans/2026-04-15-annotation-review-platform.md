...
## Next.js 16 Adjustments (IMPORTANT — supersedes any v15-specific pattern below)

This plan was drafted assuming Next.js 15. Scaffolded project is Next.js **16.2.4 + React 19.2.4**. Breaking changes from v15 to v16 that affect the rules in this plan:

1. **`params` / `searchParams` are always Promises** — any `page.tsx`, `layout.tsx`, `route.ts` that reads dynamic segments MUST be `async` and MUST `await props.params` / `await props.searchParams`. Synchronous access throws at runtime.
2. **`cookies()`, `headers()`, `draftMode()`** — must be `await`ed. `const store = await cookies()`.
3. **Type helpers** — use `PageProps<'/blog/[slug]'>`, `LayoutProps<'/blog'>`, `RouteContext<'/api/foo/[id]'>` generated by `npx next typegen`. No need to hand-type `{ params: Promise<{ ... }> }`.
4. **Turbopack is default** — `next dev` / `next build` use Turbopack. Do NOT pass `--turbopack` flags in `package.json` scripts. Use `--webpack` only if opting out. We use default.
5. **`middleware.ts` vs `proxy.ts`** — we keep `middleware.ts` to retain edge runtime for fast auth.js JWT session check. Do NOT rename to `proxy.ts` unless edge is unneeded.
6. **`next lint` removed** — `package.json` uses `"lint": "eslint"`. CI calls `pnpm lint` directly.
7. **ESLint Flat Config** — scaffolded `eslint.config.mjs` already uses flat config. Don't regenerate `.eslintrc.*`.
8. **`revalidateTag` now requires 2nd arg** — `revalidateTag('posts', 'max')`. Single-arg form is deprecated and errors in TS.
9. **`cacheLife` / `cacheTag`** — stable, no `unstable_` prefix: `import { cacheLife, cacheTag } from 'next/cache'`.
10. **`images.domains` removed** — use `images.remotePatterns` (for Vercel Blob host).
11. **React 19.2** — Canary release used. `useEffectEvent`, `ViewTransition`, `Activity` available. Not required for this plan.
12. **Node ≥ 20.9** — Vercel default runtime satisfies this; no action needed.
13. **`serverRuntimeConfig` / `publicRuntimeConfig` removed** — direct `process.env.X` access in Server Components; `NEXT_PUBLIC_*` for client. Already plan's approach.
14. **Parallel routes require `default.js`** — not used in this plan.

Every subagent implementing a task in M2-M7 will receive these rules in its prompt.

**Local reference for subagents if they need Next.js 16 API details:**
`web/node_modules/next/dist/docs/01-app/` (getting-started, guides, api-reference). Particularly useful: `api-reference/file-conventions/page.md`, `api-reference/functions/cookies.md`, `guides/authentication.md`.
